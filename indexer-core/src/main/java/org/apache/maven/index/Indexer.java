...
public interface Indexer
{
     ...
     * @param repositoryUrl  the location of the remote repository or {@code null} if this indexing context does not
     *                       need
     ...
     * @param reclaim        if indexDirectory is known to contain (or should contain) valid Maven Indexer lucene
     *                       index, and no
     *                       checks needed to be performed, or, if we want to "stomp" over existing index (unsafe to
     *                       do!).
     ...
     *                                              it has no Nexus descriptor record or it has, but the embedded
     *                                              repoId differs from the repoId
     *                                              specified from the supplied one. Never thrown if {@code reclaim}
     *                                              is {@code true}, as in that case, if
     *                                              Lucene index exists but any of those criteria above are not met,
     *                                              the existing index is overwritten,
     *                                              and equipped with proper descriptor silently.
     * @throws IllegalArgumentException             in case the supplied list of IndexCreators are having
     *                                              non-satisfiable
     *                                              dependencies.
     */
