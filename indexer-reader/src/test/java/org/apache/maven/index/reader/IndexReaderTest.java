        ...
        extends TestSupport
{
    ...
    {
        final IndexReader indexReader = new IndexReader( null, testResourceHandler( "simple" ) );
        try
        {
    ...
    public void simpleWithLocal() throws IOException
    {
        ...
        final IndexReader indexReader = new IndexReader( writableResourceHandler, testResourceHandler( "simple" ) );
        try
        {
            assertThat( indexReader.getIndexId(), equalTo( "apache-snapshots-local" ) );
            assertThat( indexReader.getPublishedTimestamp().getTime(), equalTo( 1243533418015L ) );
    ...
    public void roundtrip() throws IOException
    {
        ...
        {
            final IndexReader indexReader = new IndexReader( null, testResourceHandler( "simple" ) );
            final IndexWriter indexWriter = new IndexWriter( writableResourceHandler, indexReader.getIndexId(), false );
            try
            {
                for ( ChunkReader chunkReader : indexReader )
                {
        ...
        final IndexReader indexReader = new IndexReader( null, writableResourceHandler );
        try
        {
            assertThat( indexReader.getIndexId(), equalTo( "apache-snapshots-local" ) );
